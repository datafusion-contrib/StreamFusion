...
- [23 — A temporary planner-loader shadow, not a new Flink planner fork](23-flink-planner-loader-shadow.md) — where Spark exposes a public extension seam, Flink 2.2 does not; the distribution shadows only `PlannerModule`, preserves Flink's isolated planner classloader, injects one native program stage, then delegates to Flink's stock factory. The deliberately narrow temporary bridge is restricted to the tested Flink 2.2.0 and 2.2.1 ABIs until an upstream API replaces it.
- [31 — The Paimon sink enters through Paimon's format SPI and bundle write](31-paimon-sink-via-format-spi.md) — Arroyo and paimon-rust own the whole lake write natively; ours keeps Paimon's released Java connector for bucket rules, sequence numbers, manifests, commits, and in-job compaction, and contributes only the columnar `(partition, bucket)` shuffle and a `parquet` `FileFormatFactory` that encodes bundles natively (row-fed files delegate to Paimon's writer). Requires the JAR to sort before `paimon-flink-*`; the planner fails closed when it does not.

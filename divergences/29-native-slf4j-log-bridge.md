...
threads (librdkafka's broker threads) are attached as daemons on first use. Without a JVM — the
...
- Flink operations read logs through the TaskManager log files and web UI, which are fed by the
  ...
  per-logger level control. Routing through SLF4J gives librdkafka's stream (`librdkafka` logger)
  and native operator logs the same knobs as every other Flink logger.

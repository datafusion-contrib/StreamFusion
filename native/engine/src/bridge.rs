...
/// The engine's live-handle breakdown, e.g. `SessionAggregator=1,GroupAggregator=2` — empty once every
/// engine handle has been closed. The test harness also queries each loaded extension, so a
